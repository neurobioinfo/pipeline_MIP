...
my $OUTFILE_LJB;                # path of the annovar --filter ljb26_all output file
...
sub processArguments {
    ...
    $OUTFILE_EAS                = ($VARIANT_CALLER) ? $SUBOUTPATH . $ID . "_ANNOVAR_1000g_ean_output." . $VARIANT_CALLER : $SUBOUTPATH . $ID . "_ANNOVAR_1000g_eas_output";
    ...
    $OUTFILE_LJB                = ($VARIANT_CALLER) ? $SUBOUTPATH . $ID . "_ANNOVAR_ljb26_all_output." . $VARIANT_CALLER : $SUBOUTPATH . $ID . "_ANNOVAR_ljb26_all_output";
...
sub launchAnnovar {
    ...
    if (doIt(-elem => "1kg")) {
        my $dbtype = "1000g2014oct_all";
        my $annovarOutput = "$OUTFILE_1000G.$USEDBUILD\_ALL.sites.2014_10_dropped";
        $rO_vcf->add_header_line({key=>'INFO', ID=>'1KG', Number=>'A', Type=>'Float', Description=>"Frequency in the 1000 Genome project (2014.09 collection v5 (based on 2013.05 alignment) with including chrX and chrY data)"});
                                                                                                                                #         Autosomes from 2013_05_02 version & chromosome X from 2012.04 collection v4 (based on 2011.03 alignment)"});
        if ($RESUMEFOLDER && -e $annovarOutput) {
            # resuming ANNOVAR -filter -dbtype 1000g2014oct_all
    ...
    if (doIt(-elem => "1kg_afr")) {
        my $dbtype = "1000g2014oct_afr";
        my $annovarOutput = "$OUTFILE_AFR.$USEDBUILD\_AFR.sites.2014_10_dropped";
        $rO_vcf->add_header_line({key=>'INFO', ID=>'AF_AFR', Number=>'A', Type=>'Float', Description=>"Allele frequency of African population of the 1000 Genome project (2014.09 collection v5 (based on 2013.05 alignment) with including chrX and chrY data)"});
        if ($RESUMEFOLDER && -e $annovarOutput) {
            # resuming ANNOVAR -filter -dbtype 1000g2014oct_afr
    ...
    if (doIt(-elem => "1kg_amr")) {
        my $dbtype = "1000g2014oct_amr";
        my $annovarOutput = "$OUTFILE_AMR.$USEDBUILD\_AMR.sites.2014_10_dropped";
        $rO_vcf->add_header_line({key=>'INFO', ID=>'AF_AMR', Number=>'A', Type=>'Float', Description=>"Allele frequency of Ad Mixed American population of the 1000 Genome project (2014.09 collection v5 (based on 2013.05 alignment) with including chrX and chrY data)"});
        if ($RESUMEFOLDER && -e $annovarOutput) {
            # resuming ANNOVAR -filter -dbtype 1000g2014oct_amr
    ...
    if (doIt(-elem => "1kg_eur")) {
        my $dbtype = "1000g2014oct_eur";
        my $annovarOutput = "$OUTFILE_EUR.$USEDBUILD\_EUR.sites.2014_10_dropped";
        $rO_vcf->add_header_line({key=>'INFO', ID=>'AF_EUR', Number=>'A', Type=>'Float', Description=>"Allele frequency of European population of the 1000 Genome project (2014.09 collection v5 (based on 2013.05 alignment) with including chrX and chrY data)"});
        if ($RESUMEFOLDER && -e $annovarOutput) {
            # resuming ANNOVAR -filter -dbtype 1000g2014oct_eur
    ...
    if (doIt(-elem => "1kg_eas")) {
        my $dbtype = "1000g2014oct_eas";
        my $annovarOutput = "$OUTFILE_EAS.$USEDBUILD\_EAS.sites.2014_10_dropped";
        $rO_vcf->add_header_line({key=>'INFO', ID=>'AF_EAS', Number=>'A', Type=>'Float', Description=>"Allele frequency of East Asian population of the 1000 Genome project (2014.09 collection v5 (based on 2013.05 alignment) with including chrX and chrY data)"});
        if ($RESUMEFOLDER && -e $annovarOutput) {
            # resuming ANNOVAR -filter -dbtype 1000g2014oct_eas
    ...
    if (doIt(-elem => "1kg_sas")) {
        my $dbtype = "1000g2014oct_sas";
        my $annovarOutput = "$OUTFILE_SAS.$USEDBUILD\_SAS.sites.2014_10_dropped";
        $rO_vcf->add_header_line({key=>'INFO', ID=>'AF_SAS', Number=>'A', Type=>'Float', Description=>"Allele frequency of South Asian population of the 1000 Genome project (2014.09 collection v5 (based on 2013.05 alignment) with including chrX and chrY data)"});
        if ($RESUMEFOLDER && -e $annovarOutput) {
            # resuming ANNOVAR -filter -dbtype 1000g2014oct_sas
    ...
    if (doIt(-elem => "exac")) {
        ...
        $rO_vcf->add_header_line({key=>'INFO', ID=>'ExAC_Freq', Number=>'A', Type=>'Float', Description=>"ExAC 65000 exome allele frequency data (release 0.3 - 2015.07.29)"});
        (doIt(-elem => "exac_afr")) and $rO_vcf->add_header_line({key=>'INFO', ID=>'ExAC_AFR', Number=>'A', Type=>'Float', Description=>"ExAC 65000 exome allele frequency data in African population (release 0.3 - 2015.07.29.)"});
        (doIt(-elem => "exac_amr")) and $rO_vcf->add_header_line({key=>'INFO', ID=>'ExAC_AMR', Number=>'A', Type=>'Float', Description=>"ExAC 65000 exome allele frequency data in Admixed American population (release 0.3 - 2015.07.29.)"});
        (doIt(-elem => "exac_eas")) and $rO_vcf->add_header_line({key=>'INFO', ID=>'ExAC_EAS', Number=>'A', Type=>'Float', Description=>"ExAC 65000 exome allele frequency data in East Asian population (release 0.3 - 2015.07.29.)"});
        (doIt(-elem => "exac_fin")) and $rO_vcf->add_header_line({key=>'INFO', ID=>'ExAC_FIN', Number=>'A', Type=>'Float', Description=>"ExAC 65000 exome allele frequency data in Finnish population (release 0.3 - 2015.07.29.)"});
        (doIt(-elem => "exac_nfe")) and $rO_vcf->add_header_line({key=>'INFO', ID=>'ExAC_NFE', Number=>'A', Type=>'Float', Description=>"ExAC 65000 exome allele frequency data in Non-finnish European population (release 0.3 - 2015.07.29.)"});
        (doIt(-elem => "exac_oth")) and $rO_vcf->add_header_line({key=>'INFO', ID=>'ExAC_OTH', Number=>'A', Type=>'Float', Description=>"ExAC 65000 exome allele frequency data in Other population (release 0.3 - 2015.07.29.)"});
        (doIt(-elem => "exac_sas")) and $rO_vcf->add_header_line({key=>'INFO', ID=>'ExAC_SAS', Number=>'A', Type=>'Float', Description=>"ExAC 65000 exome allele frequency data in South Asian population (release 0.3 - 2015.07.29.)"});
        if ($RESUMEFOLDER && -e $annovarOutput) {
...
print "after if (fails): line = \"$line\"\n";
    ...
    if (doIt(-elem => "ljb") || doIt(-elem => "sift") || doIt(-elem => "pp2") || doIt(-elem => "lrt") || doIt(-elem => "mt")
            || doIt(-elem => "ma") || doIt(-elem => "fathmm") || doIt(-elem => "rsvm") || doIt(-elem => "lr") || doIt(-elem => "vest")
            || doIt(-elem => "cadd") || doIt(-elem => "gerp") || doIt(-elem => "phylop") || doIt(-elem => "siphy")) {
print STDERR "doIt = yes\n";
        my $annovarOutput = "$OUTFILE_LJB.$USEDBUILD\_ljb26_all_dropped";
        (doIt(-elem => "sift"))     and $rO_vcf->add_header_line({key=>'INFO', ID=>'SIFT_score', Number=>'A', Type=>'Float', Description=>'whole-exome SIFT scores with missing values imputed (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "sift"))     and $rO_vcf->add_header_line({key=>'INFO', ID=>'SIFT_pred', Number=>'A', Type=>'String', Description=>'whole-exome SIFT categorical prediction with missing values imputed (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "pp2"))      and $rO_vcf->add_header_line({key=>'INFO', ID=>'Polyphen2_HDIV_score', Number=>'A', Type=>'Float', Description=>'whole-exome PolyPhen 2 scores built on HumanDiv database (for complex phenotypes) (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "pp2"))      and $rO_vcf->add_header_line({key=>'INFO', ID=>'Polyphen2_HDIV_pred', Number=>'A', Type=>'String', Description=>'whole-exome PolyPhen 2 categorical prediction built on HumanDiv database (for complex phenotypes) (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "pp2"))      and $rO_vcf->add_header_line({key=>'INFO', ID=>'Polyphen2_HVAR_score', Number=>'A', Type=>'Float', Description=>'whole-exome PolyPhen 2 scores built on HumanVar database (for Mendelian phenotypes) (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "pp2"))      and $rO_vcf->add_header_line({key=>'INFO', ID=>'Polyphen2_HVAR_pred', Number=>'A', Type=>'String', Description=>'whole-exome PolyPhen 2 categorical prediction built on HumanVar database (for Mendelian phenotypes) (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "lrt"))      and $rO_vcf->add_header_line({key=>'INFO', ID=>'LRT_score', Number=>'A', Type=>'Float', Description=>'whole-exome LRT scores inccluding raw score and categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "lrt"))      and $rO_vcf->add_header_line({key=>'INFO', ID=>'LRT_pred', Number=>'A', Type=>'String', Description=>'whole-exome LRT scores inccluding categorical prediction score and categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "mt"))       and $rO_vcf->add_header_line({key=>'INFO', ID=>'MutationTaster_score', Number=>'A', Type=>'Float', Description=>'whole-exome MutationTaster scores inccluding raw score and categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "mt"))       and $rO_vcf->add_header_line({key=>'INFO', ID=>'MutationTaster_pred', Number=>'A', Type=>'String', Description=>'whole-exome MutationTaster categorical prediction inccluding raw score and categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "ma"))       and $rO_vcf->add_header_line({key=>'INFO', ID=>'MutationAssessor_score', Number=>'A', Type=>'Float', Description=>'whole-exome MutationAssessor scores inccluding raw score and categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "ma"))       and $rO_vcf->add_header_line({key=>'INFO', ID=>'MutationAssessor_pred', Number=>'A', Type=>'String', Description=>'whole-exome MutationAssessor categorical prediction inccluding raw score and categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "fathmm"))   and $rO_vcf->add_header_line({key=>'INFO', ID=>'FATHMM_score', Number=>'A', Type=>'Float', Description=>'whole-exome FATHMM scores (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "fathmm"))   and $rO_vcf->add_header_line({key=>'INFO', ID=>'FATHMM_pred', Number=>'A', Type=>'String', Description=>'whole-exome FATHMM categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "rsvm"))     and $rO_vcf->add_header_line({key=>'INFO', ID=>'RadialSVM_score', Number=>'A', Type=>'Float', Description=>'whole-exome MetaSVM scores (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "rsvm"))     and $rO_vcf->add_header_line({key=>'INFO', ID=>'RadialSVM_pred', Number=>'A', Type=>'String', Description=>'whole-exome MetaSVM categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "lr"))       and $rO_vcf->add_header_line({key=>'INFO', ID=>'LR_score', Number=>'A', Type=>'Float', Description=>'whole-exome MetaLR scores (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "lr"))       and $rO_vcf->add_header_line({key=>'INFO', ID=>'LR_pred', Number=>'A', Type=>'String', Description=>'whole-exome MetaLR categorical prediction (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "vest"))     and $rO_vcf->add_header_line({key=>'INFO', ID=>'VEST3_score', Number=>'A', Type=>'Float', Description=>'whole-exome VEST scores (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "cadd"))     and $rO_vcf->add_header_line({key=>'INFO', ID=>'CADD_raw', Number=>'A', Type=>'Float', Description=>'whole-exome CADD raw scores (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "cadd"))     and $rO_vcf->add_header_line({key=>'INFO', ID=>'CADD_phred', Number=>'A', Type=>'Float', Description=>'whole-exome CADD phred scores (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "gerp"))     and $rO_vcf->add_header_line({key=>'INFO', ID=>'GERP++_RS', Number=>'A', Type=>'Float', Description=>'whole-exome GERP++ scores (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "phylop"))   and $rO_vcf->add_header_line({key=>'INFO', ID=>'phyloP46way_placental', Number=>'A', Type=>'Float', Description=>'whole-exome PhyloP scores based on 46-way alignment placental subset (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "phylop"))   and $rO_vcf->add_header_line({key=>'INFO', ID=>'phyloP100way_vertebrate', Number=>'A', Type=>'Float', Description=>'whole-exome PhyloP scores based on 100-way alignment vertebrate subse (version 2.6 - updated 2014Sep25)'});
        (doIt(-elem => "siphy"))    and $rO_vcf->add_header_line({key=>'INFO', ID=>'SiPhy_29way_logOdds', Number=>'A', Type=>'Float', Description=>'whole-exome SiPhy scores (version 2.6 - updated 2014Sep25)'});
        if ($RESUMEFOLDER && -e $annovarOutput) {
            # resuming ANNOVAR -filter -dbtype ljb26_all
            $VERBOSE1 and printerr("\n**********\n* Resuming LJB ALL annotations using already existing file $annovarOutput\n**********\n\n");
        } else {
            my $systemCommand = "perl $ANNOVARPATH/annotate_variation.pl -filter -dbtype ljb26_all -otherinfo -buildver $USEDBUILD -indexfilter_threshold 1 -outfile $OUTFILE_LJB $ANNOINFILE $SCORESPATH/LJB/ $VERBOSE1";
            $VERBOSE1 and printerr("\n**********\n* Starting fetching of LJB ALL annotations with system command:\n* <$systemCommand>\n**********\n\n");
            my $error = system ($systemCommand);
            if ($error) {
                printerr("\n**********\n* Error (code: $error) while running system command: <$systemCommand>\n$!\n**********\n\n");
            } elsif ($VERBOSE2) {
            ...
            $VERBOSE1 and printerr("\n**********\n* Fetching of LJB ALL annotations scores done...\n**********\n\n");
        }

        # open ANNOVAR output file for processing
        open(ANNOVAR_LJB, "<$annovarOutput") || die "cannot open file $annovarOutput\n$!\n";

        # annotate noncoding variants, using the combination of chrom_start_ref in the results hash and in the *.dropped file
        $VERBOSE1 and printerr("\n**********\n* Parsing $annovarOutput to populate variant hash...\n**********\n\n");
        while (my $line = <ANNOVAR_LJB>) {         # find value of Lookup in ANNOVAR dbSNP output, and annotate variant in %results
            chomp $line;    #  $1       $2     $3    $4        $5     $6       $7          $8
            if ($line =~ /(ljb26_all)\t(.+?)\t(.+?([_\.].+)?)\t(\d+)\t(\d+)\t([ACGT\-]+)\t([ACGT\-]+)/i) {
                my $key = $3 . ":" . $5 . "-" . $6 . "_" . $7 . "/" . $8;
                my ($SIFT_score, $SIFT_pred, $Polyphen2_HDIV_score, $Polyphen2_HDIV_pred, $Polyphen2_HVAR_score, $Polyphen2_HVAR_pred, $LRT_score, $LRT_pred, $MutationTaster_score,
                    $MutationTaster_pred, $MutationAssessor_score, $MutationAssessor_pred, $FATHMM_score, $FATHMM_pred, $RadialSVM_score, $RadialSVM_pred, $LR_score, $LR_pred,
                    $VEST3_score, $CADD_raw, $CADD_phred, $GERP_RS, $phyloP46way_placental, $phyloP100way_vertebrate, $SiPhy_29way_logOdds) = split /,/, $2;
                ...
                (doIt(-elem => "rsvm"))   and $rH_data->{$key}->{RadialSVM_score}         = ($RadialSVM_score eq ".")         ? undef : $RadialSVM_score;
                (doIt(-elem => "rsvm"))   and $rH_data->{$key}->{RadialSVM_pred}          = ($RadialSVM_pred eq ".")          ? undef : $RadialSVM_pred;
                (doIt(-elem => "lr"))     and $rH_data->{$key}->{LR_score}                = ($LR_score eq ".")                ? undef : $LR_score;
                (doIt(-elem => "lr"))     and $rH_data->{$key}->{LR_pred}                 = ($LR_pred eq ".")                 ? undef : $LR_pred;
                ...
                (doIt(-elem => "phylop")) and $rH_data->{$key}->{phyloP46way_placental}   = ($phyloP46way_placental eq ".")   ? undef : $phyloP46way_placental;
                (doIt(-elem => "phylop")) and $rH_data->{$key}->{phyloP100way_vertebrate} = ($phyloP100way_vertebrate eq ".") ? undef : $phyloP100way_vertebrate;
    ...
    if (doIt(-elem => "esp6500aa")) {
        ...
        $rO_vcf->add_header_line({key=>'INFO', ID=>'ESP6500AA', Number=>'A', Type=>'Float', Description=>'alternative allele frequency in African Americans in the NHLBI-ESP project with 6500 exomes, including the indel calls and the chrY calls (db updated 2013Jan22)'});
        if ($RESUMEFOLDER && -e $annovarOutput) {
    ...
    if (doIt(-elem => "esp6500ea")) {
        ...
        $rO_vcf->add_header_line({key=>'INFO', ID=>'ESP6500EA', Number=>'A', Type=>'Float', Description=>'alternative allele frequency in European Americans in the NHLBI-ESP project with 6500 exomes, including the indel calls and the chrY calls (db updated 2013Jan22)'});
        if ($RESUMEFOLDER && -e $annovarOutput) {
    ...
    if (doIt(-elem => "gerp++elem")) {
        ...
        $rO_vcf->add_header_line({key=>'INFO', ID=>'GERP++ELEM', Number=>1, Type=>'Float', Description=>'Conserved genomic regions by GERP++ (Annovar version updated on updated 2011dec06)'});
        if ($RESUMEFOLDER && -e $annovarOutput) {
...
sub printVcf {
    ...
    while (my $rA_x = $rO_vcf->next_data_array()) {
        ...
        my ($FATHMM_pred, $FATHMM_score, $RadialSVM_pred, $RadialSVM_score, $LR_pred, $LR_score, $VEST3_score) = (undef, undef, undef, undef, undef, undef, undef);
        ...
        my ($phyloP46way_placental, $phyloP100way_vertebrate, $SiPhy_29way_logOdds, $WELLDERLY_AF, $WELLDERLY_AN) = (undef, undef, undef, undef, undef);
        ...
        foreach my $variantKey (@{$rH_vcfIndex->{$rA_x->[0] . ":" . $rA_x->[1] . "_" . $rA_x->[3] . "/" . $rA_x->[4]}}) {
            ...
            $EAS                     .= (defined $rH_data->{$variantKey}->{AF_EAS})                  ? $rH_data->{$variantKey}->{AF_EAS} . "," : "-1,";
            $SAS                     .= (defined $rH_data->{$variantKey}->{AF_SAS})                  ? $rH_data->{$variantKey}->{AF_SAS} . "," : "-1,";
            ...
            $GERP_score              .= (defined $rH_data->{$variantKey}->{'GERP++_RS'})             ? $rH_data->{$variantKey}->{'GERP++_RS'} . "," : "-1,";
        ...
        foreach my $annot ( $VC, $VFT, $VT, $Gene, $DA, $dbSNP_132, $dbSNP_138, $dbSNP_138Common, $dbSNP_138CommonAlCount, $dbSNP_138CommonAlFreq, $dbSNP_138Flagged,
                            ...
                            $GERP_score, $wgGERP, $GERPelem, $phyloP46way_placental, $phyloP100way_vertebrate, $SiPhy_29way_logOdds, $ESP6500, $ESP6500AA, $ESP6500EA,
        ...
        foreach my $annot ( $VC, $VT, $SIFT_pred, $Polyphen2_HDIV_pred, $Polyphen2_HVAR_pred, $LRT_pred, $MutationTaster_pred, $MutationAssessor_pred, $FATHMM_pred,
                            $RadialSVM_pred, $LR_pred, $CLINVAR, $COSMIC, $NCI60, $GTX_GWAS, $GTX_HGMD ) {
            if (defined $annot) {
...
#                    $annot =~ s/NIL//g;    # -> this is problem because GATK will systematiccaly remove the leading comma, impacting the segregation results !!
        ...
        foreach my $annot ( $oneKG, $CG, $AFR, $AMR, $EUR, $EAS, $SAS, $SIFT_score, $Polyphen2_HDIV_score, $Polyphen2_HVAR_score, $LRT_score, $MutationTaster_score,
                            $MutationAssessor_score, $FATHMM_score, $RadialSVM_score, $LR_score, $VEST3_score, $CADD_raw, $CADD_phred, $GERP_score, $wgGERP, $ExAC_Freq,
                            $ExAC_AFR, $ExAC_AMR, $ExAC_EAS, $ExAC_FIN, $ExAC_NFE, $ExAC_OTH, $ExAC_SAS, $phyloP46way_placental, $phyloP100way_vertebrate, $SiPhy_29way_logOdds,
                            $ESP6500, $ESP6500AA, $ESP6500EA, $GoNL, $WELLDERLY_AF, $WELLDERLY_AN ) {
...
#                    $annot =~ s/-1//g;     # -> this is problem because GATK will systematiccaly remove the leading comma, impacting the segregation results !!
        ...
        foreach my $infoTag (qw( gene snp132 snp138 snp138common snp138flagged snp142 snp142common snp142flagged 1kg 1kg_afr 1kg_amr 1kg_eur 1kg_eas 1kg_sas esp6500 esp6500aa esp6500ea nci60 cosmic
                                 exac exac_afr exac_amr exac_eas exac_fin exac_nfe exac_oth exac_sas cg clinvar gonl mirna mirnatarget rnaseq_cuff rnaseq_scri ljb
            ...
            if (!doIt(-elem => $infoTag)) {
                if ($infoTag eq "gene") {
                    ...
                    delete $infoStringHash{AF_EAS};
                } elsif ($infoTag eq "1kg_sas") {
                    delete $infoStringHash{AF_SAS};
                } elsif ($infoTag eq "1kg_afr") {
                    delete $infoStringHash{AF_AFR};
                } elsif ($infoTag eq "1kg_amr") {
                    delete $infoStringHash{AF_AMR};
                } elsif ($infoTag eq "1kg_eur") {
                    delete $infoStringHash{AF_EUR};
                    ...
                    (!doIt(-elem => "rsvm"))   and delete $infoStringHash{RadialSVM_score};
                    (!doIt(-elem => "rsvm"))   and delete $infoStringHash{RadialSVM_pred};
                    (!doIt(-elem => "lr"))     and delete $infoStringHash{LR_score};
                    (!doIt(-elem => "lr"))     and delete $infoStringHash{LR_pred};
                    ...
                    (!doIt(-elem => "phylop")) and delete $infoStringHash{phyloP46way_placental};
                    (!doIt(-elem => "phylop")) and delete $infoStringHash{phyloP100way_vertebrate};
                ...
                } elsif ($infoTag eq "rsvm") {
                    delete $infoStringHash{RadialSVM_score};
                    delete $infoStringHash{RadialSVM_pred};
                } elsif ($infoTag eq "lr") {
                    delete $infoStringHash{LR_score};
                    delete $infoStringHash{LR_pred};
                    ...
                    delete $infoStringHash{phyloP46way_placental};
                    delete $infoStringHash{phyloP100way_vertebrate};
                } elsif ($infoTag eq "siphy") {
                    delete $infoStringHash{SiPhy_29way_logOdds};
                } elsif ($infoTag eq "gtx_transfac") {
                    delete $infoStringHash{GTX_TRANSFAC_TFBS};
                } elsif ($infoTag eq "rmsk") {
                    delete $infoStringHash{REPEAT_MASKER};
                } else {
