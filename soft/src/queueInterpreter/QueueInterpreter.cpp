...
int main(int ac, char* av[]) {
	...
	try {
		...
		if (vm.count("version")) {
			cout << "Queue Interpreter, version 1.0\n";
			return EXIT_SUCCESS;
		}

		if (vm.count("resources") && verbose)
		{
			cout << "resources are: " << resources << "\n";
		}
		...
		{
			if ( verbose ) cout << "variables are: " << flush;
		...
		if ( !input_file.empty() ) {
			...
			if ( verbose ) cout << "cmd to execute: " << endl << cmd << endl;
		    ...
		    char buffer[max_buffer];
                    ...
                    out << "starting job at: " << asctime(timeinfo) << "\n";
		    ...
		    out << data;
                    time (&rawtime);
                    timeinfo = localtime (&rawtime);
                    out << "finishing job at: " << asctime(timeinfo) << "\n";
		    out.close();
		}
		else {
			cout << "Undefined input file" << endl << endl;
			cout << visible << endl <<
					"Mandatory option: " << endl << "<script>" << endl << endl;
			return EXIT_FAILURE;
		}
	}
	catch(exception& e)
