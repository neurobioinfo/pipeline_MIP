...
VERSION=1.1.1; echo "mip analysis pipeline version $VERSION"
#last updated version 2017-02-06
...
Usage() {
	...
	echo -e "\toptional arguments:\n " \
          ...
          "\t\t--molecular_tag_length    = length of molecular tag added to each read pair\n" \
...
#PRE PREPROCESSING
  ...
  #dynamically determine memory required for step 0_1 based on fastq file size; formula is .115G of memory per 1M fastq; gets rounded up to nearest multiple of 23g
  denom=23
  num=$(ls -l $F1|awk '{printf "%0.f", ($5/1024^2*.115)+1}')
  STEP_01_MEM=$(  echo $(( ( ($num / $denom) + ($num % $denom > 0) ) * $denom ))  )
  ...
  if [[ ! `find ./ -maxdepth 1 -name ${DONE}` || $SFS -eq 1 ]]; then
    preprocess=`$QUEUE $ACCOUNT  \
      ...
      -N ${SAMPLE}-${RUN}.${NAME} \
      -V \
      -v FASTQ1=$F1,FASTQ2=$F2,MIP_DESIGN_FILE=$MIP_DESIGN_FILE,MIP_DESIGN_SOURCE=$MIP_DESIGN_SOURCE,${MOLECULAR_TAG_ARGS}TRIM_PAIRED_FASTQ1=$TRIM_PAIRED_FASTQ1,TRIM_PAIRED_FASTQ2=$TRIM_PAIRED_FASTQ2,TRIM_UNPAIRED_FASTQ1=$TRIM_UNPAIRED_FASTQ1,TRIM_UNPAIRED_FASTQ2=$TRIM_UNPAIRED_FASTQ2,FASTQC_OUTPUT_DIR=$FASTQC_OUTPUT_DIR,ADAPTERS=$ADAPTERS,DONE=$DONE \
...
alignFastq() {
  ...
  if [[ -n $fastq1 && -n $fastq2 && -n $rg && -n $mip_file ]]; then
    ...
    align $fastq1 $fastq2 $mip_file $mip_source $rg $SAM $id
    if [[ ${DEPEND_ALN} ]]; then ALIGN_IDS=${ALIGN_IDS}${DEPEND_ALN}; fi
  fi
}

alignFastq $RUN1F1 $RUN1F2 $RUN1RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 1
alignFastq $RUN2F1 $RUN2F2 $RUN2RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 2
alignFastq $RUN3F1 $RUN3F2 $RUN3RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 3
alignFastq $RUN4F1 $RUN4F2 $RUN4RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 4
alignFastq $RUN5F1 $RUN5F2 $RUN5RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 5
alignFastq $RUN6F1 $RUN6F2 $RUN6RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 6
alignFastq $RUN7F1 $RUN7F2 $RUN7RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 7
alignFastq $RUN8F1 $RUN8F2 $RUN8RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 8
alignFastq $RUN9F1 $RUN9F2 $RUN9RG $MIP_DESIGN_FILE $MIP_DESIGN_SOURCE 9
